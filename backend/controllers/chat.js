...
import { de, el, fi } from "@faker-js/faker";
...
const addMembers = TryCatch(async(req, res, next)=>{
    ...
    if(!chat.groupChat) return next(new ErrorHandler("Only group chat can have members", 400));
    ...
    if(chat.members.length > 100) 
        return next(new ErrorHandler("Group members cannot exceed 100", 400));
    ...
    emitEvent(
        ...
        ALERT, 
        chat.members,
         `${allUsersName} added to the group`
        );
